...
@Component({
  selector: 'app-autocalculation-esc',
  templateUrl: './autocalculation-esc.component.html',
  styleUrls: ['./autocalculation-esc.component.css']
})
export class AutocalculationEscComponent implements OnInit {
  ...
  public displayedColumns: string[] = ['ticket_no','break_down_observed','lift_breakdown_time','type','break_down_time','restored_time','duration','ram','status','station_name','fault_type','createdDate', "actions"];
  public displayedLabelColumns: string[] = ['ticket no','CMRL comments','lift_breakdown_time','type','break down time','restored time','duration','RAM','status','station name','fault type','created Date',  'Action'];
  ...
  loadRecord() {
    ...
    this.List.forEach(element => {
        if(element.fault_type == 'Escalator not working' && element.type == '2'){
  ...
  time_calculation(){
    ...
    if(element.fault_type == 'Escalator not working'){
  ...
  get_hrs_count(){
   ...
   var  hours = 0;
    this.final_data.forEach(element => {
      if(element.lift_breakdown_time !== 'NaN' && element.fault_type == 'Escalator not working'){
